    ...
    fn poll(&mut self) {
        while let Ok(msg) = self.rx.try_recv() {
            match msg {
                ...
                JobMsg::Error(e) => {
                    self.busy = false;
                    // Elevated relaunch is macOS-only (Apple's CDC driver). Gate
                    // the call at compile time so Windows/Linux GUI builds don't
                    // hit E0425 (`relaunch_elevated_macos` is cfg'd in rome-core).
                    // open_dev() retries a stuck USB claim first; this path is
                    // the leftover genuine permission wall.
                    #[cfg(target_os = "macos")]
                    {
